...
class userActions extends Actions {
	...
	function indexAction(){
        $where = (@$_SESSION['user']['name']=='admin')?'':'user !="nobody"';
	...
	function forgotAction(){
		if(!empty($_POST['email'])){
			...
			$passw = substr(md5(uniqid(rand(5),true)),3,10);
	...
	function editAction(){
        ...
        if($user['id']!=$_SESSION['user']['id']) return $this->viewAction();
        if(@$_POST['update']){
            $nuser= array();
	...
	function blockAction(){
		$this->view = '';
       if(isset($this->request['path'][2])){
            if($this->u->update((int)$this->request['path'][2], array('name'=>'nobody'))){
            $this->response['text'] = 'ok';
            return true;
            }
       }
    ...
    function unblockAction(){
   		$this->view = '';
       if(isset($this->request['path'][2])){
            if($this->u->update((int)$this->request['path'][2], array('name'=>'user'))){
            $this->response['text'] = 'ok';
            return true;
            }
       }
