...
class Users extends Item
{
    ...
    function UserLogin($login, $passw) {
        return $this->db->SelectOne($this->table, array('where' => 'login="' . $login . '" AND password ="' . md5($passw) . '" AND name!="nobody"'));
    }
    ...
    function getAdmin() {
        return $this->db->SelectOne($this->table, array('where' => array('name' => 'admin', 'id' => 1)));
    }
    ...
    function confirmEmail($code) {
        ...
        if (! $user)
            return false;
        return $this->db->Update($this->table, array('name' => 'user'), array('code' => $code));
    }
